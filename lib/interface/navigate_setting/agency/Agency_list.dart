...
class Agency_List extends StatefulWidget {
  const Agency_List({super.key});

  @override
  State<Agency_List> createState() => _Assign_ListState();
}

class _Assign_ListState extends State<Agency_List> {
  @override
  void initState() {
    list();
    super.initState();
  ...
  Future<void> list_search() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/get_search_approved?search=$search'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonBody = jsonDecode(response.body);
        _list = jsonBody;
        setState(() {});
...
class _DataSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(
        ...
        cells: [
          ...
          DataCell(
            Text(
              '${(item['agent_type_phone'].toString() == 'null') ? '' : item['agent_type_phone'].toString()}',
              style: TextStyle(fontSize: 10),
              overflow: TextOverflow.ellipsis,
            ),
            onTap: () {},
          ...
          DataCell(
            Text(
              '${(item['agent_type_email'].toString() == 'null') ? '' : item['agent_type_email'].toString()}',
              style: TextStyle(fontSize: 10),
              overflow: TextOverflow.ellipsis,
            ),
            onTap: () {},
          ...
          DataCell(
            Text(
              '${(item['agenttype_created_date'].toString() == 'null') ? '' : item['agenttype_created_date'].toString()}',
              style: TextStyle(fontSize: 10),
              overflow: TextOverflow.ellipsis,
            ),
            onTap: () {},
                ...
                IconButton(
                    onPressed: () {
                      AwesomeDialog(
                        ...
                        btnOkOnPress: () {
                          delete(item['agenttype_id'].toString());
  ...
  Future<void> delete(id) async {
    final response = await http.delete(Uri.parse(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/delete_agency/${id.toString()}'));
    if (response.statusCode == 200) {
