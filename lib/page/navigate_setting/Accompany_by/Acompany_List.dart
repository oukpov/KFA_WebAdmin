...
import 'Edit_Acompany.dart';

class Acompany_List extends StatefulWidget {
  const Acompany_List({super.key});

  @override
  State<Acompany_List> createState() => _Assign_ListState();
}

class _Assign_ListState extends State<Acompany_List> {
  @override
  void initState() {
    list();
    super.initState();
  ...
  Future<void> list() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/get_company'));
  ...
  Future<void> list_search() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/get_search?search=$search'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonBody = jsonDecode(response.body);
        _list = jsonBody;
        setState(() {});
...
class _DataSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(
        ...
        cells: [
          ...
          DataCell(
            Text(
              item['Accompany_by_name'].toString(),
              style: TextStyle(fontSize: 10),
  ...
  Future<void> delete(id) async {
    final response = await http.delete(Uri.parse(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/delete_company/${id.toString()}'));
    if (response.statusCode == 200) {
      Navigator.pop(context);
      Navigator.push(context, MaterialPageRoute(
        builder: (context) {
          return Acompany_List();
        },
      ));
    } else {
