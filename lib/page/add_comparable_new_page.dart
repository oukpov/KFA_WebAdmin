...
import 'navigate_home/Customer/component/date_customer.dart';

class AddComparableNewPage extends StatefulWidget {
  const AddComparableNewPage(
      {super.key,
      required this.type,
...
class _HomePageState extends State<AddComparableNewPage> {
  ...
  Future<void> _loadStringList() async {
    ...
    setState(() {
      ...
      checkemail = prefs.getBool('login') ?? false;
  ...
  void checkemails(bool check) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('login', check);
  ...
  Future<void> getlist(url) async {
    var dio = Dio();
    var response = await dio.request(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/$url/${listlocalhosts[0]['agency']}',
      options: Options(
        method: 'GET',
      ),
    );
              ...
              : Container(
                  ...
                  child: PaginatedDataTable(
                    ...
                    source: _DataSource(
                        ...
                        userID: listlocalhosts[0]['agency'].toString()),
                  ),
                ),
        ],
      ),
  ...
  loginLocal(List<String> list) async {
    ...
    setState(() {
      listlocalhost = list;
      listlocalhosts = list.map((element) => element.toString()).toList();
    });
  }
  ...
  Future<void> login() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        listlocalhosts = response.data['user'];
        checkemails(checkemail);
        listlocalhost =
            listlocalhosts.map((item) => json.encode(item)).toList();
  ...
  Future<void> addComparable() async {
    protectID = int.parse(
        "${listlocalhosts[0]['agency']}${Random().nextInt(10)}${Random().nextInt(10)}${Random().nextInt(10)}${Random().nextInt(100)}");
    ...
    var data = json.encode({
      ...
      "comparabl_user": listlocalhosts[0]['agency'].toString(),
  ...
  Future<void> getAPI() async {
    var dio = Dio();
    var response = await dio.request(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/credit_Agent/${listlocalhosts[0]['agency']}',
      options: Options(
        method: 'GET',
      ),
    );
                                                            ...
                                                            listback: (value) {
                                                              setState(() {
                                                                ...
                                                                try {
                                                                  ...
                                                                  if (data_adding_correct
                                                                      .isNotEmpty) {
                                                                    if (data_adding_correct[indexs]['comparable_property_id']
                                                                            .toString() ==
                                                                        '15') {
                                                                      markerType(
                                                                          indexs,
                                                                          'l.png');
                                                                    } else if (data_adding_correct[indexs]['comparable_property_id']
                                                                            .toString() ==
                                                                        '10') {
                                                                      markerType(
                                                                          indexs,
  ...
  Future<void> updatePrice(comparableID, price, username, comparableuserID,
    ...
    var dio = Dio();
    var response = await dio.request(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/update/price/${listlocalhosts[0]['agency']}',
      options: Options(
        method: 'POST',
        headers: headers,
      ),
    ...
    if (response.statusCode == 200) {
      ...
      await addOwner(
          ...
          listlocalhosts[0]['username'].toString(),
          "Edit",
          true,
          ...
          Row(
            children: [
              ...
              (item['comparabl_user'] == listlocalhosts[0]['agency'])
                  ? EditComparableNewPage(item: item)
                  : const SizedBox(),
              Padding(
