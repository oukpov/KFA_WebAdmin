...
class _HomePageState extends State<AddComparable> {
  ...
  Future<void> _loadStringList() async {
    ...
    setState(() {
      ...
      listsangkatData = prefs.getStringList('sangkatList') ?? [];
      listCommunetData = prefs.getStringList('communeList') ?? [];
      ...
      listProperty = listpropertytData
          ...
          .cast<Map<String, dynamic>>()
          .toList();
      listsangkats = listsangkatData
          .map((item) => json.decode(item))
          .cast<Map<String, dynamic>>()
          .toList();
      listcommunes = listCommunetData
          .map((item) => json.decode(item))
          .cast<Map<String, dynamic>>()
          .toList();
    });
    ...
    if (listsangkatData.isEmpty) {
      sangkatList(widget.listlocalhosts[0]['agency'].toString());
    } else {
      for (int i = 0; i < listsangkats.length; i++) {
        listsangkatModel.add(listsangkats[i]);
      }
    }
    if (listCommunetData.isEmpty) {
      communeList(widget.listlocalhosts[0]['agency'].toString());
    } else {
  ...
  List listsangkats = [];
  List<String> listsangkatData = [];
  List<Map<dynamic, dynamic>> listsangkatModel = [];
  Future<void> sangkatList(String agencyID) async {
    var headers = {'Content-Type': 'application/json'};
    // var data = json.encode({"agency_id": 1});
    var dio = Dio();
    var response = await dio.request(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/fetch_agency/sangkat${(agencyID != '') ? "?agency_id=$agencyID" : ""}',
      options: Options(
        method: 'POST',
        headers: headers,
      ),
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        sangkatListlocal(listsangkatData);
      });
    }
  }

  List listcommunes = [];
  List<String> listCommunetData = [];
  List<Map<dynamic, dynamic>> listCommuneModel = [];
  Future<void> communeList(String agencyID) async {
    var headers = {'Content-Type': 'application/json'};
    // var data = json.encode({"agency_id": 1});
    var dio = Dio();
    var response = await dio.request(
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        listCommunetData =
            responseData.map((item) => json.encode(item)).toList();
        communeListlocal(listCommunetData);
      });
    }
  }
  ...
  sangkatListlocal(List<String> list) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setStringList('sangkatList', list);
    setState(() {
      listsangkatModel.clear();
      listsangkatData = list;
      listsangkats = list
          .map((item) => json.decode(item))
          .cast<Map<String, dynamic>>()
          .toList();
    ...
    // print('listsangkats : $listsangkats');
  }

  communeListlocal(List<String> list) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setStringList('communeList', list);
    setState(() {
      listCommuneModel.clear();
      listCommunetData = list;
      listcommunes = list
          .map((item) => json.decode(item))
          .cast<Map<String, dynamic>>()
          .toList();
  ...
  Future<void> removeSangkat() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> listLocalhostData = prefs.getStringList('sangkatList') ?? [];
    ...
    await prefs.setStringList('sangkatList', updatedList);
  }

  Future<void> removeCommune() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> listLocalhostData = prefs.getStringList('communeList') ?? [];
    List<Map<String, dynamic>> listlocalhost = listLocalhostData
        .map((item) => json.decode(item) as Map<String, dynamic>)
        .toList();
    // Find the index of the current user using both email and password
    ...
    List<String> updatedList =
        listlocalhost.map((item) => json.encode(item)).toList();
    await prefs.setStringList('communeList', updatedList);
  }
  ...
  Future<void> insertDistrict() async {
    try {
      ...
      var dio = Dio();
      var response = await dio.request(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/insert_agency/sangkat',
        options: Options(
          method: 'POST',
          headers: headers,
        ),
      ...
      if (response.statusCode == 200) {
        List<dynamic> responseData = response.data;
        setState(() {
          // listsangkats = response.data;
          listsangkatData =
              responseData.map((item) => json.encode(item)).toList();
        });
        await sangkatListlocal(listsangkatData);
  ...
  Future<void> insertCommunes() async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      ...
      if (response.statusCode == 200) {
        ...
        await communeListlocal(listCommunetData);
        // print(json.encode(response.data));
      } else {
        // print(response.statusMessage);
      }
                                                                            ...
                                                                            SizedBox(
                                                                              ...
                                                                              child: TextFormField(
                                                                                ...
                                                                                decoration: InputDecoration(
                                                                                  ...
                                                                                  labelText: "District",
                                                                                  labelStyle: const TextStyle(color: kTextLightColor),
                                                                                  prefixIcon: const Icon(Icons.location_on_rounded, color: kImageColor),
                                                                                  contentPadding: const EdgeInsets.symmetric(vertical: 8),
                                                                                  focusedBorder: OutlineInputBorder(
